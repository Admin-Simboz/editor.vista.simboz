<template>
        ...
        <Content style="display: flex; height: calc(100vh - 64px)">
          <div v-if="state.show" :class="`left-bar ${state.toolsBarShow ? 'show-tools-bar' : ''}`">
            <Menu :active-name="state.menuActive" accordion @on-select="showToolsBar" width="65px">
              <MenuItem v-for="item in menuItems" :key="item.name" :name="item.name" class="menu-item">
  ...
  const state = reactive({
    ...
    ruler: false,
    menuItems: [
      { name: 1, icon: 'md-book', text: 'templates' },
      { name: 2, icon: 'md-images', text: 'elements' },
      { name: 3, icon: 'ios-leaf-outline', text: 'material.cartoon' },
      { name: 4, icon: 'md-reorder', text: 'layers' },
    ],
  });
  ...
  class CanvasHandler {
    constructor(canvasId) {
      ...
      this.canvasEditor = this.initializeEditor(this.canvas);
    }
